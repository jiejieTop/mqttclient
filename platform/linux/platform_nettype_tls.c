 ...
 * @LastEditTime : 2020-01-13 09:30:07
...
static int platform_nettype_tls_init(network_t* n, nettype_tls_params_t* nettype_tls_params)
{
...
#if defined(MBEDTLS_X509_CRT_PARSE_C)
    if (NULL != n->network_params.network_tls_params.ca_crt) {
        n->network_params.network_tls_params.ca_crt_len = strlen(n->network_params.network_tls_params.ca_crt);
        LOG_I("ca len is %d", n->network_params.network_tls_params.ca_crt_len);
        if ((rc = mbedtls_x509_crt_parse(&(nettype_tls_params->ca_cert), (const unsigned char *)n->network_params.network_tls_params.ca_crt,
            (n->network_params.network_tls_params.ca_crt_len + 1)))) {
    ...
    if (n->network_params.network_tls_params.cert_file != NULL && n->network_params.network_tls_params.key_file != NULL) {
            if ((rc = mbedtls_x509_crt_parse_file(&(nettype_tls_params->client_cert), n->network_params.network_tls_params.cert_file)) != 0) {
            LOG_E("load client cert file failed returned 0x%x", rc<0?-rc:rc);
            return PLATFORM_ERR_SSL_CERT;
        }

        if ((rc = mbedtls_pk_parse_keyfile(&(nettype_tls_params->private_key), n->network_params.network_tls_params.key_file, "")) != 0) {
            LOG_E("load client key file failed returned 0x%x", rc<0?-rc:rc);
            return PLATFORM_ERR_SSL_CERT;
        }
    } else {
        LOG_I("cert_file/key_file is empty! | cert_file = %s | key_file = %s", n->network_params.network_tls_params.cert_file, n->network_params.network_tls_params.key_file);
    }
#else
	if (n->network_params.network_tls_params.psk != NULL && n->network_params.network_tls_params.psk_id !=NULL) {
        const char *psk_id = n->network_params.network_tls_params.psk_id;
        rc = mbedtls_ssl_conf_psk(&(nettype_tls_params->ssl_conf), (unsigned char *)n->network_params.network_tls_params.psk, n->network_params.network_tls_params.psk_length,
                                    (const unsigned char *) psk_id, strlen( psk_id ));
    } else {
        LOG_I("psk/pskid is empty! | psk = %s | psd_id = %s", n->network_params.network_tls_params.psk, n->network_params.network_tls_params.psk_id);
    }
...
int platform_nettype_tls_connect(network_t* n)
{
    ...
    mbedtls_ssl_conf_read_timeout(&(nettype_tls_params->ssl_conf), n->network_params.network_tls_params.timeout_ms);
...
#ifndef AUTH_MODE_CERT
    if(n->network_params.network_tls_params.psk != NULL) {
